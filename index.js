...
async function fetchStockData() {
    ...
    try {
        const stockData = await Promise.all(tickersArr.map(async (ticker) => {
            const url = `https://polygon-api-worker.aka-arka-sarkar.workers.dev/?ticker=${ticker}&startDate=${dates.startDate}&endDate=${dates.endDate}`
...
async function fetchReport(data) {
    ...
    try {
        const url = 'https://openai-api-worker.aka-arka-sarkar.workers.dev'
        ...
        if (response.ok) {
            ...
            renderReport('API Error: ' + (data.error || 'Unknown error'));
        }
    } catch (err) {
        console.error(err.message)
